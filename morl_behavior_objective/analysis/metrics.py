...
from utils import create_ground_truth_dst
...
def compute_intrinsic_dimension(embedding_space, k=None):
    ...
    try:
        ...
        return {"intrinsic_dimension": float(intrinsic_dim), "k_neighbors": k}
    except Exception:
        return {"intrinsic_dimension": float("nan"), "k_neighbors": k if k is not None else None}
...
def compute_mantel_test(embedding_space, objective_space, perms=2000, seed=42, save_histogram=False, image_dir=None):
    ...
    try:
        ...
        observed_corr, _ = spearmanr(dist_embedding[upper_tri_indices], dist_objective[upper_tri_indices])
        ...
        for _ in range(perms):
            ...
            null_corr, _ = spearmanr(dist_embedding_perm[upper_tri_indices], dist_objective[upper_tri_indices])
        ...
        if save_histogram and image_dir:
            ...
            ax.axvline(observed_corr, color="red", linestyle="--", linewidth=2, label=f"Observed: {observed_corr:.3f}")
...
def compute_neighborhood_metrics(embedding_space, objective_space, k=None, add_jitter=True):
    ...
    try:
        ...
        try:
            trustworthiness_score = trustworthiness(objective_space_std, embedding_space, n_neighbors=k)
        except Exception:
            # Fallback to k=1 if needed
            trustworthiness_score = trustworthiness(objective_space_std, embedding_space, n_neighbors=1)
...
def compute_procrustes_analysis(embedding_space, objective_space, seed=42):
    ...
    try:
        ...
        rho_aligned, p_val_aligned = spearmanr(dist_emb_aligned[upper_tri_indices], dist_obj_aligned[upper_tri_indices])
...
def compute_geodesic_metrics(embedding_space, objective_space):
    ...
    try:
        ...
        rho_geodesic, p_val_geodesic = spearmanr(
            geodesic_embedding[upper_tri_indices], geodesic_objective[upper_tri_indices]
        )
...
def compute_order_correlation(embedding_space, objective_space, k_neighbors=None):
    ...
    try:
        ...
        return {"order_correlation": float("nan"), "k_neighbors": k_neighbors, "objective_1d": None, "embedding_1d": None}


def compute_shepard_stress(embedding_space, objective_space, save_plot=False, image_dir=None, embed_dim=None):
    ...
    try:
        ...
        return {"stress": stress, "alpha": alpha, "shepard_plot_path": plot_path}
    except Exception:
        return {"stress": float("nan"), "alpha": float("nan"), "shepard_plot_path": None}


def compute_lipschitz_metrics(
    embedding_space, objective_space, k_neighbors=None, save_histograms=False, image_dir=None, embed_dim=None
):
    ...
    try:
        ...
        if k_neighbors >= 1 and n_samples > 1:
            ...
            ratios_obj_to_emb[mask_obj] = embedding_distances_to_obj_neighbors[mask_obj] / (distances_obj[mask_obj] + eps)
            ...
            ratios_emb_to_obj[mask_emb] = objective_distances_to_emb_neighbors[mask_emb] / (distances_emb[mask_emb] + eps)
    ...
    try:
        ...
        for i in range(n_policies):
            for j in range(i + 1, n_policies):
                ...
                if (label_a, label_b) in pairs_definition or (label_b, label_a) in pairs_definition:
...
def compute_all_metrics(
    ...
    results["mantel"] = compute_mantel_test(
        embedding_space, objective_space, save_histogram=save_plots, image_dir=image_dir
    )
    ...
    results["shepard"] = compute_shepard_stress(
        embedding_space, objective_space, save_plot=save_plots, image_dir=image_dir, embed_dim=embed_dim
    )

    # Lipschitz metrics
    results["lipschitz"] = compute_lipschitz_metrics(
        embedding_space, objective_space, save_histograms=save_plots, image_dir=image_dir, embed_dim=embed_dim
    )
...
if __name__ == "__main__":
    ...
    for dim_i in dim:
        ...
        all_returns = np.array(all_returns)
        print("Transformer embeddings and returns loaded successfully:")
