...
def run_encoder_only_training_sa(
    ...
    if getattr(encoder, "model_type", "") == "BE_SA_VDT":
        raise ValueError(
            "Encoder-only loop not supported for BE_SA_VDT (variational) encoders. Use alpha=0 in the AE loop instead."
        )
    ...
    optimizer = th.optim.Adam(list(encoder.parameters()) + list(infomax_loss_fn.discriminator.parameters()), lr=lr)
    ...
    sigma = (
        ...
        if env_id == "Walker2dv4"
        else 0.5
        if env_id == "Humanoidv4"
        else 0.5
    )
    tau = (
        0.1
    ...
    for epoch in epoch_pbar:
        encoder.train()
        infomax_loss_fn.discriminator.train()
        total_loss = 0.0
        ...
        for batch in dataloader:
            ...
            L_min, L_max = (
                (8, 12)
                if env_id in ("TwoLakesFishing-v0", "Traj2d")
                else (8, 16)
                if env_id in ("Reacher-v4", "Pusherv4")
                else (8, 16)
                if env_id in ("Walker2dv4", "Humanoidv4")
                else (8, 16)
            )
            T = states.shape[1]
            ...
            loss_infomax = (
                infomax_loss_fn(cls1, norm1[:, 1:, :], local_mask1) + infomax_loss_fn(cls2, norm2[:, 1:, :], local_mask2)
            ) / 2.0
...
def inference_on_dataloader_cdec_sa(encoder, cluster_centroids, dataloader, trajectory_manager, device, index_offset=0):
...
def fit_hdbscan_seen(X_seen: np.ndarray, granularity: float, seed: int):
    ...
    model = HDBSCAN(min_cluster_size=min_cluster_size, min_samples=min_samples, metric="cosine").fit(X_seen)
...
def assign_or_flag_online(X_online: np.ndarray, registry: dict, maha_p: float = 0.99):
    ...
    maha_vals = []
    for x, cid in zip(Xn, best_cids, strict=False):
    ...
    for i, (dc, cid) in enumerate(zip(d_cos, best_cids, strict=False)):
...
def spawn_new_clusters_from_buffer(novel_points: np.ndarray, min_cluster_size: int = 5) -> list[dict]:
    ...
    hdb_micro = HDBSCAN(
        min_cluster_size=min_cluster_size, min_samples=max(1, int(math.sqrt(min_cluster_size))), metric="cosine"
    ).fit(novel_points)
...
def plot_novelty_metrics(y_true, continuous_scores, hard_mask, out_prefix, title="", show=True):
    ...
    mean_ap, (ap_lo, ap_hi) = _bootstrap_ci(
        average_precision_score, np.asarray(y_true), np.asarray(continuous_scores), n_boot=200
    )
    ...
    try:
        ...
        ax_cal.set_xlabel("Predicted score (binned)")
        ax_cal.set_ylabel("Observed fraction positive")
    ...
    return {
        "roc_auc": roc_auc,
        "roc_auc_boot_mean": mean_roc,
        "roc_auc_ci": (roc_lo, roc_hi),
        "ap": ap,
        "ap_boot_mean": mean_ap,
        "ap_ci": (ap_lo, ap_hi),
...
def visualize_controller_output(
    ...
    for cid in np.unique(labels_seen):
        ...
        if is_3d:
            ax.scatter(
                Z_seen[mask, 0], Z_seen[mask, 1], Z_seen[mask, 2], c=[color], s=20, alpha=0.6, marker=marker, label=label
            )
        else:
    ...
    if len(Z_online) > 0:
        ...
        if np.any(novel_mask):
            ...
            scores_norm = (novelty_scores[novel_mask] - novelty_scores[novel_mask].min()) / (
                novelty_scores[novel_mask].max() - novelty_scores[novel_mask].min() + 1e-6
            )
            sizes = 50 + 200 * scores_norm

            if is_3d:
                p = ax.scatter(
                    Z_online[novel_mask, 0],
                    Z_online[novel_mask, 1],
                    Z_online[novel_mask, 2],
                    c="red",
                    s=sizes,
                    marker="*",
                    label="Online (Novel)",
                ...
                p = ax.scatter(
                    Z_online[novel_mask, 0], Z_online[novel_mask, 1], c="red", s=sizes, marker="*", label="Online (Novel)"
                )
    ...
    if registry:
        ...
        if is_3d:
            ...
            ax.scatter(
                Z_centers[:, 0], Z_centers[:, 1], c="black", s=250, marker="P", edgecolor="white", label="Cluster Centers"
            )
...
def visualize_classic_scalers_on_flat_states(
    ...
    scalers = {
        ...
        "Quantile-Uniform": QuantileTransformer(
            n_quantiles=min(1000, len(seen_states)), output_distribution="uniform", random_state=seed
        ),
        "Quantile-Normal": QuantileTransformer(
            n_quantiles=min(1000, len(seen_states)), output_distribution="normal", random_state=seed
        ),
        "Power(Yeo-Johnson)": PowerTransformer(method="yeo-johnson", standardize=True),
        "Normalizer(L2)": Normalizer(norm="l2"),
    }
    print(
        f"[Scaler Viz] Seen={seen_states.shape}, Online={online_states.shape}, fit_mode={fit_mode}, umap_metric={umap_metric}"
    )

    # UMAP reducer (fit on seen-only or both, see below)
    reducer = umap.UMAP(random_state=seed, n_neighbors=n_neighbors, min_dist=min_dist, n_components=2, metric=umap_metric)
...
def get_label_display_properties(
    HIGH_CONTRAST_PREDICTED_COLORS, label_value, args_env, current_color_label_mapping, label_type="true"
):
    ...
    if label_type == "true":
        ...
        if args_env.Highway:
            ...
            if label_value == 10:
                ...
                color = "g"
            name = current_color_label_mapping.get(color, f"Behavior {label_value}")
        elif args_env.Traj2d or args_env.Reacherv4 or args_env.Pusherv4:
            if label_value == 10:
                color = "tab:blue"
            elif label_value == 11:
                color = "tab:red"
