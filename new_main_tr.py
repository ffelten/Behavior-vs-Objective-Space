...
from morl_behavior_objective.methods.behaviorencoder_utils import expand_interleaved_mask
...
class TrajectoryDecoder(nn.Module):
    def __init__(self, emb_dim, state_dim, action_dim, max_len, spec_norm=False):
        ...
        self.net = nn.Sequential(
            ...
            if not spec_norm
            else parametrize.spectral_norm(nn.Linear(1024, self.out_features)),
        )
...
def aggregate_and_visualize_policy_embeddings(embeddings, policies, emb_dim, save_path=None):
    policy_latents = {}
    unique_policies = np.unique(policies)
    for pid in unique_policies:
...
def main():
    ...
    if args.DeepSeaTreasureConcave or args.DeepSeaTreasureSmooth or args.DeepSeaTreasureLeftRight:
        ...
        file_names = [
            "both_resources_dodging_E1_through_E2.json",
            "both_resources_through_both_Es.json",
            "diamond_home.json",
            "gold_dodging_all_Es_in_12_steps.json",
            "gold_through_E1_both_ways.json",
            "gold_through_E1_only_once.json",
        ]
    ...
    model_name = (
        model_name.replace(".pt", f"_decorr{int(args.decorr_weight)}.pt") if args.decorr_weight > 0.0 else model_name
    )
    model_name = (
        model_name.replace(".pt", "_lower_t.pt")
        if args.temperature == 0.1
        else model_name.replace(".pt", "_two_t.pt")
        if args.temperature == 0.2
        else model_name.replace(".pt", "_pf_t.pt")
        if args.temperature == 0.05
        else model_name.replace(".pt", "_h_t.pt")
    ...
    viz_path = (
        viz_path.replace(".png", "_lower_t.png")
        if args.temperature == 0.1
        else viz_path.replace(".png", "_two_t.png")
        if args.temperature == 0.2
        else viz_path.replace(".png", "_pf_t.png")
        if args.temperature == 0.05
        else viz_path.replace(".png", "_h_t.png")
        if args.temperature == 0.01
        else viz_path
    )
    ...
    json_path = os.path.join(
        embeddings_folder_path, f"{name_env}_{args.emb_dim}D_l{args.nlayers}_embeddings_e{args.epochs}.json"
    )
    json_path = (
        json_path.replace(".json", f"_decorr{int(args.decorr_weight)}.json") if args.decorr_weight > 0.0 else json_path
    )
    json_path = (
        json_path.replace(".json", "_lower_t.json")
        if args.temperature == 0.1
        else json_path.replace(".json", "_two_t.json")
        if args.temperature == 0.2
        else json_path.replace(".json", "_pf_t.json")
        if args.temperature == 0.05
        else json_path.replace(".json", "_h_t.json")
        if args.temperature == 0.01
        else json_path
    )
    ...
    policy_objectives = np.array([obj_feats_list[np.where(true_labels == pid)[0][0]] for pid in unique_pids])
    ...
    trust_w = trustworthiness(D_obj_precomputed, D_emb_wasserstein, n_neighbors=k_neighbors, metric="precomputed")

    # Continuity: How many of the k-nearest neighbors in D_emb_w
    # are preserved as neighbors in D_obj? (Just swap the matrices)
    cont_w = trustworthiness(D_emb_wasserstein, D_obj_precomputed, n_neighbors=k_neighbors, metric="precomputed")
